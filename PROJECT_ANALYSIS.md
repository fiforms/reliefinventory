...
| Order filling / picking | ✅ Built (Aug 27 2026) — `OrderFillingController` + `OrderFilling.vue`, both a live-scan path and a paper pick-sheet batch path on one shared fill-record backend (see CLAUDE.md's Order Filling section). |
| BOL creation & printing | ✅ Built (Aug 27 2026), PDF generation only — `OrderController::bolPdf`, generatable once an order is Filled, `bol_number` assigned on first print and reused on reprint. Driver/carrier assignment and the three-signature chain-of-custody are hand-entry blank lines on the printed form (no data source for those yet); signed-BOL upload and the rest of the Ready to Ship/Shipped lifecycle are still not built — see `order-fulfillment-lifecycle-design` memory. |
| Log received orders / upload signed BOLs | ✅ Built (Aug 27 2026) — Shipping page (`/shipping`) assigns a driver to a Filled order (→ Ready to Ship) and marks it Shipped when it leaves the dock; the driver-facing Driver Portal (`/driver-portal`, phone+PIN, no account) uploads the signed BOL (→ Delivered); a manager then reviews it on the Shipping page (optional drag-crop before approving) — Approve moves it to the real terminal **Completed** status, Reject sends it back to Shipped for the driver to redo. See `order-fulfillment-lifecycle-design` memory. |
