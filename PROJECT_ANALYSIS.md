...
| ~~4~~ | ~~Dead menu links.~~ **PARTIALLY FIXED.** `/reports/inventory`, and (new, Aug 2026) `/reports/dashboard` and `/reports/sitrep`, are now real pages with real data. Nothing 404s anymore either way — the rest (`/order-filling`, `/reports/orders`, `/reports/flow`, `/reports/donors`, `/reports/customers`, `/setup/users`) render an honest "Coming Soon" placeholder (`ComingSoon.vue`) instead of dead-linking, per Phase 0 item 6 below. Still genuinely unbuilt: Order Filling and the orders/flow/donors/customers reports. | `database/migrations/2025_02_07_032546_fill_menu_tables.php`, `routes/web.php` |
...
| Reports (inventory, flow, donors, customers, outstanding orders) | 🟡 Partial, much improved (Aug 2026, Part 8) — **Inventory Report** (on-screen + PDF) and a **Warehouse Dashboard** (internal, full detail) + **Situation Report** (external, restricted, PDF-exportable) are built and live. Flow, donor-quality, and customer reports, plus Outstanding Orders, are still dead "Coming Soon" links. |
